...
# 'a' is append, so old data will not be erased in case you want to add.
...
for i in range(0, 90):
    name = nfl_roster_body.find_all('tr')[i].find_all('td')[0].find('a', title=True)['title'].encode('ascii','ignore')
    number = nfl_roster_body.find_all('tr')[i].find_all('td')[1].text.strip()
    position = nfl_roster_body.find_all('tr')[i].find_all('td')[2].text.strip()
    height = nfl_roster_body.find_all('tr')[i].find_all('td')[3].text.strip()
    weight = nfl_roster_body.find_all('tr')[i].find_all('td')[4].text.strip()
    age = nfl_roster_body.find_all('tr')[i].find_all('td')[5].text.strip()
    exp = nfl_roster_body.find_all('tr')[i].find_all('td')[6].text.strip()
    college = nfl_roster_body.find_all('tr')[i].find_all('td')[7].text.strip()
    # store all to the csv
